...
using System.IO;
using System.Linq;
using game_inazuma_eleven_go.Properties;
using Kontract.Interface;

namespace game_inazuma_eleven_go
{
    public enum Scenes
    {
        Dialog,
    ...
    public class Handler : IGameHandler
    {
        ...
        static Lazy<XF> fontInitializer = new Lazy<XF>(() => new XF(new MemoryStream(Resources.ft_nrm))); //Get the arquive ft_nrm from the Resources folder
        ...
        Bitmap textBox = new Bitmap(Resources.blank_top); //Get the arquive blank_top from the Resources folder
        Bitmap csBTM = new Bitmap(Resources.cutscene_bottom); //Get the arquive blank_top from the Resources folder
            ...
            using (Graphics gfx = Graphics.FromImage(img))
            {
                ...
                switch (Enum.Parse(typeof(Scenes), Properties.Settings.Default.Scene))
                {
                    case Scenes.Dialog:
                        ...
                        rectText = new RectangleF(0, 10, 360, 180);
                        colorDefault = Color.FromArgb(255, 255, 255, 255); // Define the color of the text inside the baloon
                        break;

                    case Scenes.CutsceneBottom:
                        gfx.DrawImage(csBTM, 0, 0);
                        rectText = new RectangleF(0, 10, 300, 100);
